"""Validates the diagonal (tie-mass) inflation fix (src/models/diagonal_inflation.py)
on top of the full current-best pipeline. Builds the final RECORDED-score
joint (regulation, inflated, then convolved with the real OT/SO layer --
same construction as Cycle 10's `_build_final_joint`, now with the
diagonal correctly inflated first) and validates SU/Brier/total-MAE/
margin-MAE, plus the Var(T)/E(T) consistency check the external review
flagged (Sec10): the fitted joint's implied total-goals dispersion should
reproduce the independently-measured ~0.97-0.98 ratio (Sec4.11), or the fit
...
from src.models.diagonal_inflation import fit_theta, inflate_diagonal
...
def _build_final_joint(lambda_home_reg: float, lambda_away_reg: float, theta: float,
                        p_home_wins_ot: float, max_goals: int = 12) -> np.ndarray:
    reg_joint = dc_adjusted_joint(lambda_home_reg, lambda_away_reg, rho=0.0, max_goals=max_goals)  # rho=0 -> plain indep Poisson
    reg_joint = inflate_diagonal(reg_joint, theta)
...
def run_validation(min_season: int = 20102011, max_season: int = DEV_MAX_SEASON,
    ...
    """`theta_override`: if given, skips fitting theta and uses this value directly
    -- e.g. 1.0 for the no-inflation ablation (does the regulation-ratio
    reconstruction + OT/SO layer alone move any metric, with the diagonal
    term switched off)."""
    ...
    for row in base.itertuples():
        final_joint = _build_final_joint(row.lambda_home_reg, row.lambda_away_reg, theta, p_home_wins_ot)
    ...
    # base and treated are row-aligned (treated built via itertuples() over base with no filtering).
    return base.reset_index(drop=True), treated, theta, p_home_wins_ot
...
def paired_bootstrap(base: pd.DataFrame, treated: pd.DataFrame, n_resamples: int = 5000,
    ...
    """Paired bootstrap over games: resample game indices WITH replacement (same
    resample applied to both models each draw), compare mean(metric) diffs.
    treated - base: negative Brier/MAE diff and positive SU diff both mean
    treated is better. 95% CI = 2.5/97.5 percentile of the resampled diffs."""
...
if __name__ == "__main__":
    base, r, theta, p_home_wins_ot = run_validation()
    print(f"validated {len(r)} dev-set games, theta={theta:.4f}, p_home_wins_ot={p_home_wins_ot:.4f}")

    for label, df in [("BASELINE (drift_adjusted_poisson, production)", base), ("DIAGONAL-INFLATED (candidate)", r)]:
    ...
    print("\npaired bootstrap (5000 resamples), diagonal-inflated MINUS baseline "
          "(negative brier/MAE diff = improvement, positive SU diff = improvement):")
    boot = paired_bootstrap(base, r)
    for key in ("su", "brier", "total_ae", "margin_ae"):
        b = boot[key]
        print(f"  {key}: mean_diff={b['mean_diff']:.5f}, 95% CI [{b['ci_low']:.5f}, {b['ci_high']:.5f}]")

    # Var(T)/E(T) consistency check (external review, Sec10) -- CORRECTED (2026-07-23,
    # per a follow-up review catching a like-for-like mismatch): Sec4.11's "0.97-0.98" was
    # the MARGINAL per-side (home, away) variance/mean ratio, not Var(H+A)/E(H+A) -- a
    # different statistic. The population-level model figure must also use the law of
    # total variance (between-game lambda heterogeneity contributes real variance to the
    # pooled population, not just each game's own conditional variance), and the REAL
    # target must be measured directly from actual data, not carried over from Sec4.11.
    actual_total = r["actual_home"] + r["actual_away"]
    real_var_over_mean = actual_total.var(ddof=0) / actual_total.mean()

    pop_var_total = r["var_total"].mean() + r["exp_total"].var(ddof=0)  # E[Var(T|g)] + Var[E(T|g)]
    pop_exp_total = r["exp_total"].mean()
    model_ratio = pop_var_total / pop_exp_total

    print(f"\nCORRECTED Var(T)/E(T) check (population-level, law of total variance, "
          f"same statistic both sides):")
    print(f"  REAL empirical Var(actual_total)/E(actual_total): {real_var_over_mean:.4f} "
          f"(measured directly on these {len(r)} games -- NOT the Sec4.11 marginal figure)")
    print(f"  MODEL-implied (diagonal-inflated candidate): {model_ratio:.4f}")
