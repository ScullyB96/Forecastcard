    ...
    for g in game_rows:
        ...
        for team, team_margin in [(g["home"], margin), (g["away"], -margin)]:
            ...
            for p in team_roster.itertuples():
                ...
                prop_rows.append({
                    "team": team, "player": p.player_name, "position": p.position,
                    "proj_targets": round(proj_targets, 1), "proj_carries": round(proj_carries, 1),
                    "td_probability": round(td_prob, 3),
                })

    props_df = pd.DataFrame(prop_rows).sort_values("td_probability", ascending=False)
    print(props_df.head(20).to_string(index=False))
    props_df.to_parquet(DATA_PROCESSED / f"props_{pred_season}_wk{pred_week}.parquet", index=False)

    print(f"\nsaved predictions_{pred_season}_wk{pred_week}.parquet, props_{pred_season}_wk{pred_week}.parquet")
